...
| `onEnter`                 | func    | null                | Callback triggered when the tooltip appears: `(event?: Event) => void`. Receives the native DOM event that triggered the transition — `MouseEvent` on hover, `FocusEvent` on focus, `TouchEvent` when using `touchBehavior` — or `undefined` when shown programmatically via the `open` prop.                                        |
| `onLeave`                 | func    | null                | Callback triggered when the tooltip disappears: `(event?: Event) => void`. Receives the native DOM event that triggered the transition — `MouseEvent` on mouseleave, `FocusEvent` on focusout, `KeyboardEvent` on Escape, `TouchEvent` when using `touchBehavior` — or `undefined` when hidden programmatically via the `open` prop. |
