...
| `onEnter`                 | func    | null                | Callback triggered when the tooltip appears: `(event?: Event) => void`. Receives the native DOM event that triggered the transition (e.g. `MouseEvent` on hover, `FocusEvent` on focus), or `undefined` when the tooltip is shown programmatically via the `open` prop.                                                              |
| `onLeave`                 | func    | null                | Callback triggered when the tooltip disappears: `(event?: Event) => void`. Receives the native DOM event that triggered the transition (e.g. `MouseEvent` on leave, `KeyboardEvent` on Escape), or `undefined` when the tooltip is hidden programmatically via the `open` prop.                                                      |
| `onPlacementChange`       | func    | null                | Callback triggered when the tooltip auto-flips to a different position due to viewport overflow: `(from: TooltipPosition, to: TooltipPosition) => void`. `from` is the declared position, `to` is the actual rendered position. Useful to sync custom arrow classes or layout indicators with the effective placement. Fires regardless of `containerClassName`. |
...
| `portal`                  | boolean | true                | When `true` (default), the tooltip is appended to `document.body` and positioned with `position: fixed`, avoiding stacking-context issues caused by CSS `transform`, `filter`, or `perspective` on ancestor elements. When `false`, the tooltip is appended as a direct child of the target element and positioned with `position: relative` — matching the pre-v4 behavior. |
