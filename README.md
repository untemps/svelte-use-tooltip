...
| `open`                    | boolean | undefined           | Programmatically control tooltip visibility. `true` shows the tooltip and locks it open — hover/focus events and Escape are ignored until `open` is released. `false` hides the tooltip and prevents hover/focus from showing it. `undefined` (or not passed) restores normal hover/focus behavior. Has no effect when `disabled` is `true`. |
